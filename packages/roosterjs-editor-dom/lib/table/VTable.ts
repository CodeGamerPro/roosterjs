...
export default class VTable {
    ...
    row: number;
    col: number;
    private trPrototype: HTMLTableRowElement[] = [];
    ...
    constructor(node: HTMLTableElement | HTMLTableCellElement) {
        ...
        trs.forEach((tr, rowIndex) => {
            this.trPrototype[rowIndex % 2] = tr;
    ...
    writeBack() {
        if (this.cells) {
            ...
            this.cells.forEach((row, r) => {
                let tr = VTable.cloneNode(this.trPrototype[r % 2] || this.trPrototype[0]);
...
function toArray<T extends HTMLElement>(collection: HTMLCollectionOf<T>): T[] {
    let result: T[] = [];
